--- 
:format: :textile
:title: Developing a Rails model using BDD and RSpec, Part 1
:published_on: Tue Aug 29 13:15:00 UTC 2006
---
...
bc(ruby).. class UserTest < Test::Unit::TestCase
  def test_create
...
p. Now ask yourself how many times have you sat back and asked yourself _why_ you are writing the above test?
...
bc(ruby).. class User < ActiveRecord::Base
end
...
bc(shell).. ~/mygreatapp/ $ sudo gem install rspec
...
p. Replace X, Y and Z in the above with the version of RSpec that you are using. If you have any problems, refer to the "full instructions":http://rspec.rubyforge.org/tools/rails.html.
...
bc(shell).. $ ./script/generate rspec_model User

p. This will create a new user.rb file for our model, just like the normal Rails script/generate model command, but it will also create an accompanying spec file in the spec/ directory. If you open up the created user_spec.rb file, you will see a stub context ready and waiting.
...
bc(ruby).. context "A user (in general)" do
  setup do
    @user = User.new
  end
...
p. That's not bad, but it could be better. We've expressed a requirement in our code but we haven't said anything about the behaviour of a User object. What about this instead:
...
p. That's better. Not only have we expressed that our user must have a username, but we've also expressed what behaviour should be expected from the User model if it doesn't have one; it should be invalid. Let's fill this spec in, so we have a failing spec:
...
bc(ruby).. class AddUsersTable < ActiveRecord::Migration
  def self.up
...
p. We've also specified that our user _must_ have an email address, so lets add a spec for that:
...
p. That's simple enough to implement:
...
p. Now, we don't actually have a password column in our users table, but we need somewhere to store the cleartext password before it gets encrypted. A standard Ruby instance variable will do. Here's the code to make it pass:
...
p. h3. Refactoring towards cleaner, clearer specifications
...
p. There, thats much DRYer, more expressive and easier to maintain. If our valid attributes ever change, we only need to change them in one place. However, we haven't sacrificed readability in the name of DRY, which is very important with any tests/specs.

Finally, lets add a spec for our password length:

bc(ruby).. specify "should be invalid if password is not between 6 and 12 characters in length" do
  @user.attributes = valid_user_attributes.except(:password)
...
bc(ruby)..   specify "should be valid with a full set of valid attributes" do
